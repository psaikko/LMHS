...
int InputReader::readCardinalityConstraint(istream& inputF) {
	...
	else if(coding == "CARDN") {
		codingN = CardinalityConstraint::CODING_NETWORK;
	}
	else if(coding == "CARDC") {
		codingN = CardinalityConstraint::CODING_COUNTER;
	}
	else if(coding == "CARDB") {
		codingN = CardinalityConstraint::CODING_BEST;
	}
	else {
		return 1;
	}
